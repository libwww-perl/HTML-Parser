...
my @placements = (["nul-space", "\0 "], ["nul-gt", "\0>"]);
...
for my $elem (@elements) {
    for my $p (@placements) {
        my ($label, $mid) = @$p;
        push @cases,
            ["$elem/$label", $elem, "</$elem$mid<img src=x onerror=boom>"];
    }
    push @cases, ["$elem/unclosed", $elem, "<img src=x onerror=boom>"];
}

plan tests => scalar @cases;

for my $case (@cases) {
    my ($label, $elem, $rest) = @$case;
    ...
    $parser->parse("<$elem>safe$rest");
    $parser->eof;

    is(
        join("|", @events),
        "start=$elem|text=safe$rest|end=$elem",
        "$label: injected markup stays raw text inside $elem",
    );
}
